...
/*   Updated: 2025/07/28 17:43:47 by yingzhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_chars(t_map *map, char c, int i, int j)
{
	...
	if (c == 'M')
	{
		...
		map->enemy_y = i;
		if (++map->enemy != 1)
			free_and_exit(-1, map, "More than 1 enemy in map");
	}
	if (c == 'E' && ++map->exit != 1)
...
void	validate_map(t_map *map)
{
	...
	if (map->enemy < 1)
		free_and_exit(-1, map, "Less than 1 enemy in map");
}
